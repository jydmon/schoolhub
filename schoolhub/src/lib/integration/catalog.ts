...
export const HUB_CATALOG: ConnectorTemplate[] = [
  ...
  // --- Government / DfE ---
  { key: "dfe-find-use-api", name: "DfE — Find and use an API", provider: "Department for Education", category: "government", description: "DfE 'Find and use an API' service (e.g. the DfE Information API). Register your application on the DfE portal and subscribe to the required API (sandbox first, then production) to get a subscription key. Auth is OAuth2 client-credentials (Azure AD token endpoint + scope) plus the APIM subscription key. Configurable shell — we never call an API you haven't authorised.", connectionType: "rest", authMethod: "client_credentials", supportedObjects: ["Establishment", "*"], supportedOperations: ["read", "import"], status: "custom", setupComplexity: "high", defaultFrequency: "daily", requiresProviderCredentials: true, configFields: [{ key: "baseUrl", label: "API URL", type: "url", required: true, help: "The 'API URL' from the DfE API-subscriptions page, e.g. https://api-customerengagement.platform.education.gov.uk/dfe/ (sandbox has its own URL)." }, { key: "tokenEndpoint", label: "Token endpoint", type: "url", required: true, help: "The OAuth2 'Token endpoint' from the subscription page, e.g. https://login.microsoftonline.com/<tenant>/oauth2/token." }, { key: "scope", label: "Scope / resource", type: "text", required: true, help: "The 'Scopes' value from the subscription page, e.g. https://dfeb2cprod.onmicrosoft.com/DFEInfo/ReadAll." }, { key: "clientId", label: "Client ID (Application ID)", type: "text", required: true, help: "From the DfE application's 'Application details' page." }, { key: "clientSecret", label: "Client secret", type: "secret", secret: true, required: true, help: "Generated on the DfE application's 'Client secrets' page." }, { key: "subscriptionKey", label: "Subscription key", type: "secret", secret: true, required: true, help: "The APIM subscription key from the 'API subscriptions' page (sent as the Ocp-Apim-Subscription-Key header)." }, { key: "scheduleCron", label: "Schedule (cron, optional)", type: "text" }], docsUrl: "https://find-and-use-an-api.education.gov.uk/", icon: "🏛️" },
];
