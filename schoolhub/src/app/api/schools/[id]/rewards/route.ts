...
export async function POST(req: Request, { params }: Params) {
  try {
    ...
    const reward = await prisma.rewardRecord.create({
      data: { schoolId: params.id, studentId: i.studentId, type: i.type, points: i.points ?? (positive ? 1 : 0), category: i.category || null, note: i.note || null, teacherName: i.teacherName || null, source: i.source || "Behaviour system", positive, at: i.at ? new Date(i.at) : new Date() },
    });

    // Notify guardians per their reward preferences.
    ...
    await recordAudit({ action: AUDIT.REWARD_INGEST, schoolId: params.id, actorUserId: ctx.userId, actorEmail: ctx.email, targetType: "RewardRecord", targetId: reward.id, metadata: { type: i.type } });
