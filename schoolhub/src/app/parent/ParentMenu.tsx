...
export default function ParentMenu() {
  ...
  const [dietary, setDietary] = useState("all"); // all | vegetarian | vegan
  ...
  const filtered = useMemo(() => items.filter((m) => {
    ...
    if (avoid.length) {
      const its = (m.allergens || "").toLowerCase();
      if (avoid.some((a) => its.includes(a))) return false;
    }
    return true;
  ...
  const byDay = useMemo(() => {
    ...
    for (const m of filtered) { (g[m.day] = g[m.day] || {}); (g[m.day][m.meal] = g[m.day][m.meal] || []).push(m); }
    return g;
  }, [filtered]);
  const days = Object.keys(byDay).sort((a, b) => (DAY_ORDER[a] ?? 9) - (DAY_ORDER[b] ?? 9) || a.localeCompare(b));
    ...
    <>
        ...
        {allergens.length > 0 && (
          <div style={{ marginTop: 10 }}>
            ...
            <div className="chips" style={{ marginTop: 4 }}>
              {allergens.map((a) => (
                <button key={a} className={avoid.includes(a) ? "" : "secondary"} onClick={() => toggleAvoid(a)} style={{ textTransform: "capitalize" }}>{ALLERGEN_ICON[a] || "•"} {a}</button>
              ))}
      ...
      {days.length === 0 ? (
        ...
        <div className="panel" key={day}>
          <h2 style={{ fontSize: 16, marginTop: 0 }}>{DAY_LABEL[day] || day}</h2>
          {Object.keys(byDay[day]).sort((a, b) => (MEAL_ORDER[a] ?? 9) - (MEAL_ORDER[b] ?? 9)).map((meal) => (
            <div key={meal} style={{ marginBottom: 12 }}>
              ...
              <div style={{ display: "grid", gap: 8 }}>
                {byDay[day][meal].sort((a: any, b: any) => a.course.localeCompare(b.course)).map((m: any) => {
